...
//			return string.Empty;
	...
	public static string GetBodyFrameAsCsv(SensorData sensorData, ref BodyFrameData bodyFrame, ref long liRelTime, ref float fUnityTime, char delimiter)
	{
		...
		for(int i = 0; i < sensorData.bodyCount; i++)
		{
			...
			if(bodyFrame.bodyData[i].bIsTracked != 0)
			{
				...
				sbBuf.Append(bodyFrame.bodyData[i].liTrackingID +1).Append(delimiter);

				for(int j = 0; j < sensorData.jointCount; j++)
				{
					KinectInterop.JointData jointData = bodyFrame.bodyData[i].joint[j];
                   // KinectInterop.JointType[] name = (KinectInterop.JointType[])Enum.GetValues(typeof(KinectInterop.JointType));
					...
					if(jointData.trackingState != TrackingState.NotTracked)
					{
                        ...
                        //sbBuf.AppendFormat("{0:F3}", name[j].ToString()).Append(delimiter);
    ...
    public static bool SetBodyFrameFromCsv(string sCsvLine, SensorData sensorData, ref BodyFrameData bodyFrame, ref Matrix4x4 kinectToWorld)
	{
		...
		for(int i = 0; i < bodyCount; i++)
		{
			...
			if(bIsTracked != 0 && alCsvParts.Length >= (iIndex))
			{
				long.TryParse(alCsvParts[iIndex], out liTrackingID);
				iIndex++;
			...
			bodyFrame.bodyData[i].liTrackingID = liTrackingID+1;
			...
			if(bIsTracked != 0)
			{
				...
				for(int j = 0; j < jointCount; j++)
				{
					...
					if(alCsvParts.Length >= (iIndex + 1))
					{
                       ...
                       // Debug.Log("fichero joint " +j +"  "+ jointData.kinectPos);
                       // Debug.Log("Transformado joint " + j +"  " + jointData.position);
    ...
    // sets current body frame from the given csv line. returns true on success, false otherwise
    public static bool SetBodyFrameFromCsvAndPoll(string sCsvLine, SensorData sensorData, ref BodyFrameData bodyFrame, ref Matrix4x4 kinectToWorld,bool ignoreZCoordinates)
    {
        // check for invalid sensor data and for same frame time
        if (sensorData == null)
            return false;
        ...
        sensorData.sensorInterface.PollBodyFrame(sensorData, ref bodyFrame, ref kinectToWorld, ignoreZCoordinates);
        // update body frame data
        ...
        while (bodyFrame.bodyData[free].bIsTracked != 0 && free < bodyCount)
        {
            free++;
        }

        bool ultima_verificacion = false;
        int iIndex = 4;
        for (int i = 0; i < bodyCount; i++)
        {
            ...
            if (bodyFrame.bodyData[i].bIsTracked == 0)
            {
                ...
                if (bIsTracked != 0)
                {
                    ...
                    bodyFrame.bodyData[free].bIsTracked = bIsTracked;
                    bodyFrame.bodyData[free].liTrackingID = liTrackingID + 1;
                    bodyFrame.bodyData[free].bisCsv = 1;
                    ...
                    if (jointCount != sensorData.jointCount)
                    {
                        ...
                        for (int j = jointCount; j < sensorData.jointCount; j++)
                        {
                            bodyFrame.bodyData[free].joint[j].trackingState = TrackingState.NotTracked;
                        }
                    }

                    // update joints' data
                    bodyFrame.bodyData[i].bisCsv = 1;
                    for (int j = 0; j < jointCount; j++)
                    {
                        KinectInterop.JointData jointData = bodyFrame.bodyData[free].joint[j];
                        int iTrackingState = 0;
                        ...
                        if (alCsvParts.Length >= (iIndex + 1))
                        {
                            ...
                            {
                                // set body position
                                bodyFrame.bodyData[free].position = jointData.position;
                                bodyFrame.bodyData[free].orientation = jointData.orientation;
                            }
                        }

                        bodyFrame.bodyData[free].joint[j] = jointData;
                    }
                }
                else
                    bodyFrame.bodyData[i].bisCsv = 0;
