...
{
    private bool Mostrar_Mensaje_Calibrar = true;

    public GameObject boton_analisis; 

    public void SetMostrar_Mensaje_Calibrar(bool a)
    {
        Mostrar_Mensaje_Calibrar = a;
    }
    /// <summary>
...
//		LeftHandCursor,
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked , Gesture_Action mov, GameObject[] Cubeman0_bones)
	{
...
//							}
			...
			case Gestures.Run:
				...
				case 0:  // gesture detection - phase 1
                    ...
                    if ((jointsPos[leftKneeIndex].y - jointsPos[rightKneeIndex].y) > 0.1f)
					{
						SetGestureJoint(ref gestureData, timestamp, leftKneeIndex, jointsPos[leftKneeIndex]);
						gestureData.progress = 0.3f;
					}
                        Debug.Log("Empezando a correr");  
					break;
				...
				case 1:  // gesture complete
					...
					else
					{
                        ...
                        Debug.Log("Estoy corriendo pierna derecha");
                        break;
				...
				case 2:  // gesture complete
					...
					else
					{
                        ...
                        Debug.Log("Estoy corriendo pierna izquierda");
                        break;
				}
				break;
            ...
            case Gestures.Move:
                ...
                {
                    case 0:  // gesture detection - phase 1
                             //instancia Gesture_Action
                        i = 1;
                        KinectRecorderPlayer.Instance.inpose = false;

                        if (mov.Comparar && mov.Getmovimientos() !=null)
                        {
                            Vector3[] movimientos = (Vector3[])mov.Getmovimientos()[0];
                            while (i < 25 && inpose)
                            {
                                //inpose = ((Cubeman0_bones[i].transform.localPosition.x <= (movimientos[i].x + mov.Error_Margin)) && (Cubeman0_bones[i].transform.localPosition.x >= (movimientos[i].x - mov.Error_Margin))) &&
                                //         ((Cubeman0_bones[i].transform.localPosition.y <= (movimientos[i].y + mov.Error_Margin)) && (Cubeman0_bones[i].transform.localPosition.y >= (movimientos[i].y - mov.Error_Margin)));

                                inpose = ((Cubeman0_bones[i].transform.localPosition.x <= (movimientos[i].x + mov.Error_Margin)) && (Cubeman0_bones[i].transform.localPosition.x >= (movimientos[i].x - mov.Error_Margin))) &&
                                          ((Cubeman0_bones[i].transform.localPosition.y <= (movimientos[i].y + mov.Error_Margin)) && (Cubeman0_bones[i].transform.localPosition.y >= (movimientos[i].y - mov.Error_Margin))) &&
                                          ((Cubeman0_bones[i].transform.localPosition.z <= (movimientos[i].z + mov.Error_Margin)) && (Cubeman0_bones[i].transform.localPosition.z >= (movimientos[i].z - mov.Error_Margin)));
                                i++;
                            }

                            if (inpose)
                            {
                                GameObject.Find("Registro Movimiento Usuario").GetComponent<Registrar_Movimientos>().BorrarRegistro();
                                gestureData.timestamp = timestamp + 4f;
                                gestureData.progress = 0.3f;
                                gestureData.state++;
                                Mostrar_Mensaje_Calibrar = false;
                                StartCoroutine(Empezando_movimiento(mov));
                                ...
                                {
                                    StartCoroutine(Mensaje(count, seconds,mov));
                                    seconds++;
                                    count--;
                                }
                            ...
                            if (Mostrar_Mensaje_Calibrar) {
                                Debug.Log("Calibrando la posicion inicial");
                                //mov.infoText_Movimiento.text = "Calibrando la posicion inicial";
                                mov.cambiar_calibrar_texto("Calibrando la posicion inicial");
                            }
                    ...
                    default:  // gesture complete
                        

                         // mov.infoText_Movimiento.text = "MOVIMIENTO EN CURSO";
                         if ((timestamp - gestureData.timestamp) < mov.Gesture_Action_Time + 3.0f)
                            {
                            ...
                            i = 1;
                                inpose = true;
                            Debug.Log("FASE DE MOVIMIENTO " + gestureData.state);
                            Vector3[] movimientos = (Vector3[])mov.Getmovimientos()[gestureData.state];
                                ...
                                if (inpose)
                                {
                                    ...
                                    if (gestureData.state == mov.Getmovimientos().Count - 1)
                                    {
                                        SetGestureCancelled(ref gestureData);
                                        KinectRecorderPlayer.Instance.inpose = false;
                                        KinectRecorderPlayer.Instance.move_finished = true;
                                        mov.Comparar = false;
                                        gestureData.complete = true;
                                        Debug.Log("MOVIMIENTO REALIZADO");
                                        //mov.infoText_Movimiento.text = "MOVIMIENTO REALIZADO";
                                        mov.cambiar_calibrar_texto("MOVIMIENTO REALIZADO");
                                        Mostrar_Mensaje_Calibrar = false;
                                        //Para asignarle la animación win
                                        StartCoroutine(Win_animation(mov));
                                    ...
                                    gestureData.state += mov.Fase_rango;
                                    if (gestureData.state > mov.Getmovimientos().Count - 1)
                                        gestureData.state = mov.Getmovimientos().Count - 1;
                            ...
                            {
                            //   
                                //Se guarda el estado del movimiento fracasado    
                               GameObject.Find("Registro Movimiento Usuario").GetComponent<Registrar_Movimientos>().Estado_del_moviento_fracasado= gestureData.state;
                                // cancel the gesture
                                SetGestureCancelled(ref gestureData);
                                KinectRecorderPlayer.Instance.inpose = false;
                                mov.Comparar = false;
                                Debug.Log("MOVIMIENTO FRACASADO");
                                //mov.infoText_Movimiento.text = "MOVIMIENTO FRACASADO";
                                 mov.cambiar_calibrar_texto("MOVIMIENTO FRACRASADO");
                                

                            
                                Mostrar_Mensaje_Calibrar = true;
                                //Para asignarle la animación lose
                                 GameObject.Find("Avatar recorder").GetComponent<Animator>().SetTrigger("Lose");
                                    StartCoroutine(Borrar_Meensaje(mov));

                            //activar boton analisis
                            boton_analisis.SetActive(true);
                            boton_analisis.GetComponent<ActivarBotonAnalisis>().activar_boton();
                        }
                        Debug.Log("FASE SIGUIENTE");
    ...
    private IEnumerator Win_animation(Gesture_Action mov)
    {
        yield return new WaitForSeconds(1.5f);
        GameObject.Find("Avatar recorder").GetComponent<Animator>().SetTrigger("Win");
        mov.infoText_Movimiento.text = "";
        StartCoroutine(Borrar_Meensaje(mov));
    }

    private IEnumerator Empezando_movimiento(Gesture_Action mov)

    {
       
        
        yield return new WaitForSeconds(4f);
        if (KinectRecorderPlayer.Instance.IsPlaying())
            //mov.infoText_Movimiento.text = "MOVIMIENTO EN CURSO";
             mov.cambiar_calibrar_texto("MOVIMIENTO EN CURSO");
        KinectRecorderPlayer.Instance.inpose = true;
        KinectRecorderPlayer.Instance.calibracion = false;


    }
    private IEnumerator Mensaje (int second_mensaje,float second, Gesture_Action mov)
    {
        yield return new WaitForSeconds(second);
        if (KinectRecorderPlayer.Instance.IsPlaying())
        //mov.infoText_Movimiento.text = "Empezando movimiento en " + second_mensaje ;
        mov.cambiar_calibrar_texto("Empezando movimiento en " + second_mensaje);

    }

    private IEnumerator Borrar_Meensaje(Gesture_Action mov)
    {
        yield return new WaitForSeconds(1f);
        //mov.infoText_Movimiento.text = "";
        mov.cambiar_calibrar_texto("");
    }
}
