...
//		}
    ...
    /// <summary>
    /// Gets the user recorded ID .
    /// </summary>
    /// <returns>The user ID by index.</returns>
    /// <param name="i">The user index.</param>
    public Int64 GetUserIdRecorded()
    {
        int i = 0;
        Int64 result = 0;
        while (i < KinectInterop.Constants.MaxBodyCount && (result == 0))
        {
            if (bodyFrame.bodyData[i].bisCsv == 1)
            {
                int Userid = GetUserIndexById(bodyFrame.bodyData[i].liTrackingID);
                    if (Userid >=0 && i < KinectInterop.Constants.MaxBodyCount)
                    result = aUserIndexIds[Userid];
            }
            i++;
        }
    ...
    /// <summary>
    /// Gets the user normal ID .
    /// </summary>
    /// <returns>The user ID by index.</returns>
    /// <param name="i">The user index.</param>
    public Int64 GetUserNormalId()
    {
        int i = 0;
        Int64 result = 0;
        while (i < KinectInterop.Constants.MaxBodyCount && (result == 0))
        {
            if (bodyFrame.bodyData[i].bisCsv == 0)
            {
                int Userid = GetUserIndexById(bodyFrame.bodyData[i].liTrackingID);
                if (Userid >= 0 && i < KinectInterop.Constants.MaxBodyCount)
                    result = aUserIndexIds[Userid];
    ...
    /// <summary>
    /// Gets if the user is recorded  .
    /// </summary>
    /// <returns>The user ID by index.</returns>
    /// <param name="i">The user index.</param>
    public bool IsRecordedUser(Int64 i)
    { 
    int j = 0;
    bool result = false;
        if (i != 0)
        {
            while (j < KinectInterop.Constants.MaxBodyCount && (!result))
            {

                    result = (bodyFrame.bodyData[j].liTrackingID == i) && (bodyFrame.bodyData[j].bisCsv == 1);
...
//		}
    ...
    /// <summary>
    /// Enables or displables the compare mode.
    /// </summary>
    /// <param name="bEnabled">If set to <c>true</c> enables the compare mode.</param>
    public void EnableCompareMode(bool bEnabled)
    {
        if (sensorData != null)
        {
            sensorData.isCompareModeEnabled = bEnabled;
            sensorData.playModeData = string.Empty;
    ...
    /// </summary>
    /// <returns><c>true</c> if the play mode is enabled; otherwise, <c>false</c>.</returns>
    public bool isCompareModeEnabled()
    {
        if (sensorData != null)
        {
            return sensorData.isCompareModeEnabled;
        }
    ...
    /// <summary>
    /// Sets the body frame from the given csv line to compare.
    /// </summary>
    /// <returns><c>true</c> on success, <c>false</c> otherwise.</returns>
    /// <param name="sLine">The body frame as csv line.</param>
    public bool SetBodyFrameDataCompare(string sLine)
    {
        if (sensorData != null && sensorData.isCompareModeEnabled)
        {
            sensorData.playModeData = sLine;
            return true;
        }
...
//			KinectInterop.ShutdownKinectSensor();
		...
		if(kinectInitialized)
		{
			   ...
			   (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
	        {
				if(usersMapRect.width == 0 || usersMapRect.height == 0)
				{
					...
					Rect cameraRect = Camera.allCameras[0] != null ? Camera.allCameras[0].pixelRect : new Rect(0, 0, Screen.width, Screen.height);
                    ...
                    usersMapRect = new Rect(50f, cameraRect.height- 30f, 200f, -200f);
			...
			else if(computeColorMap && displayColorMap)
			{
				if(usersClrRect.width == 0 || usersClrRect.height == 0)
				{
                    ...
                    usersClrRect = new Rect(50f, cameraRect.height - 30f, 200f, -200f);
	...
	private void UpdateKinectStreams()
	{
		if(kinectInitialized)
		{
            ...
            if (!sensorData.isCompareModeEnabled)
            ...
            else
            {
                ...
                sensorData.newBodyFrame = KinectInterop.SetBodyFrameFromCsvAndPoll(sensorData.playModeData, sensorData, ref bodyFrame, ref kinectToWorld, ignoreZCoordinates);
	...
	private void ProcessBodyFrameData()
	{
		...
		for(int i = 0; i < sensorData.bodyCount; i++)
		{
			   ...
			   // && (maxTrackedUsers < 0 || trackedUsers < maxTrackedUsers))
			{
                ...
                //if (!addedUsers.Contains(userId))
                //{
                //    if (bodyData.bisCsv == 1)
                //    {
                //        	addedUsers.Insert(1, userId);
		...
		if(!alUserIds.Contains(userId))
		{
			if(CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
			{
				...
				for(int i = 0; i < avatarControllers.Count; i++)
				{
                    ...
                    {
                        if (avatar.AvatarRecorder)
                        {
                            avatar.playerId = GetUserIdRecorded();
                            avatar.SuccessfulCalibration(GetUserIdRecorded());
                        }
                        ...
                        else
                        {
                            ...
                            avatar.playerId = GetUserNormalId();
                            avatar.SuccessfulCalibration(GetUserNormalId());
...
//						else
		...
		if(Time.realtimeSinceStartup >= gesturesTrackingAtTime[UserId])
		{
			...
			for(int g = 0; g < listGestureSize; g++)
			{
					...
					!IsConflictingGestureInProgress(gestureData, ref gesturesData))
				{
                    ...
                    CubemanController cubeman = null;
                    if (GameObject.Find("Cubeman0"))
                     cubeman = GameObject.Find("Cubeman0").GetComponent<CubemanController>();
                    GameObject[] Cubeman0_bones = null;
                    if (cubeman)
                    {
                       Cubeman0_bones = cubeman.GetBones();
                    }
                    
                    gestureManager.CheckForGesture(UserId, ref gestureData, Time.realtimeSinceStartup, 
						ref playerJointsPos, ref playerJointsTracked, movimiento, Cubeman0_bones);
					gesturesData[g] = gestureData;
	...
	protected virtual bool CheckForCalibrationPose(Int64 UserId, int bodyIndex, KinectGestures.Gestures calibrationGesture)
	{
        ...
        Gesture_Action movimiento = Gesture_Action.Instance;
        CubemanController cubeman = GameObject.Find("Cubeman0").GetComponent<CubemanController>();
        GameObject[] Cubeman0_bones = cubeman.GetBones();
        gestureManager.CheckForGesture(UserId, ref gestureData, Time.realtimeSinceStartup, 
			ref playerJointsPos, ref playerJointsTracked,movimiento, Cubeman0_bones);
